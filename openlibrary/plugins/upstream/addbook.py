...
class works_autocomplete(delegate.page):
    ...
    def GET(self):
        ...
        if embedded_olid:
            solr_q = f'key:"/works/{embedded_olid}"'
        else:
        ...
        if embedded_olid and not docs:
            ...
            key = f'/works/{embedded_olid}'
            if work := web.ctx.site.get(key):
...
class authors_autocomplete(delegate.page):
    ...
    def GET(self):
        ...
        if embedded_olid:
            solr_q = f'key:"/authors/{embedded_olid}"'
        else:
            prefix_q = f"{q}*"
        ...
        if embedded_olid and not docs:
            ...
            key = f'/authors/{embedded_olid}'
            if author := web.ctx.site.get(key):
                docs = [author.as_fake_solr_record()]

        for d in docs:
            d['works'] = [d.pop('top_work')] if 'top_work' in d else []
