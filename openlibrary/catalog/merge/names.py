...
def flip_name(name):
    ...
    return None if not m else f'{m.group(2)} {m.group(1)}'
...
def flip_marc_name(marc):
    ...
    if normalize(first_parts[-1]) not in titles:
        ...
        return f'{remove_trailing_dot(m.group(2))} {m.group(1)}'
    if len(first_parts) > 2 and normalize(first_parts[-2]) == normalize(m.group(1)):
        return ' '.join(first_parts[:-1])
...
def match_marc_name(marc1, marc2, last_name_only_ok):
    ...
    if not m1:
        if m2 and m1_normalized == normalize(m2.group(1)):
            ...
            return False
    if not m2:
        return last_name_only_ok if m2_normalized == normalize(m1.group(1)) else False
    if m1_normalized == normalize(
        f'{m2.group(2)} {m2.group(1)}'
    ) or m2_normalized == normalize(f'{m1.group(2)} {m1.group(1)}'):
        return True
    if not (
        m1.group(1).endswith((f' {m2.group(1)}', f'.{m2.group(1)}'))
        or m2.group(1).endswith((f' {m1.group(1)}', f'.{m1.group(1)}'))
    ):
    ...
    if amazon_title(marc1_first_parts, marc2_first_parts):
        return True
    return bool(amazon_title(marc2_first_parts, marc1_first_parts))
...
def match_name2(name1, name2):
    ...
    if amazon_title(n1_parts, n2_parts):
        return True
    return bool(amazon_title(n2_parts, n1_parts))


def match_surname(surname, name):
    if name.endswith((f' {surname}', f'.{surname}')):
        return True
    surname = surname.replace(' ', '')
    return bool(name.endswith((f' {surname}', f'.{surname}')))


def amazon_spaced_name(amazon, marc):
    len_amazon = len(amazon)
    if len_amazon not in [30, 31]:
        return False
    ...
    if compare_parts(marc_first_parts, amazon_first_parts):
        return True
    return bool(match_seq(amazon_first_parts, marc_first_parts))
...
def match_name(amazon, marc, last_name_only_ok=True):
    ...
    if amazon_normalized == normalize(
        ...
        return last_name_only_ok
    if amazon_normalized == normalize(f'{m.group(2)} {surname}'):
        if verbose:
            print('match')
        return True
    ...
    if not match_surname(surname, amazon):
        ...
        return False
    marc_first_parts = split_parts(m.group(2))
    amazon_first_parts = split_parts(amazon[:-(len(m.group(1)) + 1)])
    if compare_parts(marc_first_parts, amazon_first_parts):
        if verbose:
            print("match")
        return True
